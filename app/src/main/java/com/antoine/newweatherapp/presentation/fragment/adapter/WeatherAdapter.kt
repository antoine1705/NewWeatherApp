package com.antoine.newweatherapp.presentation.fragment.adapter
...
import com.antoine.newweatherapp.databinding.ItemWeatherInformationBinding
import com.antoine.newweatherapp.extensions.get

class WeatherAdapter :
    ListAdapter<WeatherAdapter.AdapterItem, RecyclerView.ViewHolder>(differUtils) {
    ...
    class WeatherInfoViewHolder(private val binding: ItemWeatherInformationBinding) :
        ...
        fun bindData(weather: AdapterItem.WeatherUI) = with(binding.tvWeatherInfo) {
            ...
            spannable.append(
                context.getString(
                    R.string.text_temperature_title, weather.tempAvg
                ) + "\n"
            )
            spannable.append(context.getString(R.string.text_pressure_title) + weather.pressure + "\n")
            spannable.append(
                context.getString(
                    R.string.text_humidity_title, weather.humidity
                ) + "\n"
            )
...
private val differUtils =
    AsyncDifferConfig.Builder(object : DiffUtil.ItemCallback<WeatherAdapter.AdapterItem>() {
        override fun areItemsTheSame(
            oldItem: WeatherAdapter.AdapterItem, newItem: WeatherAdapter.AdapterItem
        ): Boolean {
            return when (oldItem) {
