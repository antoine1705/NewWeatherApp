package com.antoine.newweatherapp.data.local.dao
...
interface WeatherDao {
    ...
    @Query(
        "SELECT Weather.* FROM Weather INNER JOIN City " + "ON Weather.city_id = City.id " + "WHERE City.short_name = :cityName " + "AND Weather.date >= :startDate " + "AND Weather.date <= :endDate"
    )
    suspend fun findWeatherByCityName(
        cityName: String, startDate: Long, endDate: Long
    ): List<WeatherEntity>
